...
def load_base_graph(json_path):
    ...
    for node in nodes:
        print(node)
        cluster_label = G.nodes[node].get('cluster_label', -1)
        cluster_labels.append(cluster_label)
    data.cluster_label = torch.tensor(cluster_labels, dtype=torch.long)
    ...
    # data.cluster_label = 
    # torch.tensor([type2index[node_type] for node_type
...
def generate_graph_dataset(base_graph, fft_base_path, total_samples, graph_label=0, padding_types=("cap", "ind", "res", "ind")):
    ...
    for inst_type in unique_types:
        try:
            arr = load_fft_data(inst_type.lower(), fft_base_path)
    ...
    for _ in range(total_samples):
        ...
        data.cluster_label = torch.tensor(base_graph.cluster_label, dtype=torch.long)
...
class ClusterClsDataset(InMemoryDataset):
    '''
...
if __name__ == "__main__":
    ...
    def show_dataset(dataset):
        ...
        # Print cluster label example
        # print("Node source label example:", dataset[0].node_label)
        print("Graph cluster data example:", dataset[0].cluster_label)
        ...
        # print("Node source label example:", dataset[0].node_label)
        print("Graph cluster data example:", dataset[1].cluster_label)

        # print("Graph label example:", dataset[1].graph_label)
        # print("Node label example:", dataset[1].node_label)


    # node_cls_dataset  = NodeClsDataset(root='./det_dataset', name='train', args=args)
    # graph_cls_dataset = GraphClsDataset(root='./det_dataset', name='train', args=args)
    #cluster_cls_dataset = ClusterClsDataset(root='./det_dataset', name='train', args=args)
    # show_dataset(node_cls_dataset)
    # show_dataset(graph_cls_dataset)
    #show_dataset(cluster_cls_dataset)

    args.n_samples = 10
    # # node_cls_dataset  = NodeClsDataset(root='./det_dataset', name='val', args=args)
    # # graph_cls_dataset = GraphClsDataset(root='./det_dataset', name='val', args=args)
    cluster_cls_dataset = ClusterClsDataset(root='./det_dataset_timing', name='val', args=args)
    # # show_dataset(node_cls_dataset)
    # # show_dataset(graph_cls_dataset)
    show_dataset(cluster_cls_dataset)

    #args.n_samples = 20
    # node_cls_dataset  = NodeClsDataset(root='./det_dataset', name='test', args=args)
    # graph_cls_dataset = GraphClsDataset(root='./det_dataset', name='test', args=args)
    #cluster_cls_dataset = ClusterClsDataset(root='./det_dataset', name='test', args=args)
    # show_dataset(node_cls_dataset)
    # show_dataset(graph_cls_dataset)
    #show_dataset(cluster_cls_dataset)
