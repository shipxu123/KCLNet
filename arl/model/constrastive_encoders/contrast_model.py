...
class TriBranchContrast(torch.nn.Module):
    def __init__(self, loss: Loss, mode: str, intraview_negs: bool = False, **kwargs):
        super(TriBranchContrast, self).__init__()
        self.loss = loss
        self.mode = mode
        self.sampler = get_sampler(mode, intraview_negs=intraview_negs)
        self.kwargs = kwargs

    def forward(self, g0=None, g1=None, g2=None, extra_pos_mask=None, extra_neg_mask=None):
        if self.mode == 'G2G':
            assert g0 is not None and g1 is not None and g2 is not None
            anchor1, sample1, pos_mask1, neg_mask1 = self.sampler(anchor=g0, sample=g1, neg_sample=g2)
            anchor2, sample2, pos_mask2, neg_mask2 = self.sampler(anchor=g1, sample=g0, neg_sample=g2)
        else:  # global-to-local
            raise NotImplementedError("Not implement L2L and Global-to-local for TriBranchConstrast")
