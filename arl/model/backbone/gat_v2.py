    ...
    def __init__(
        ...
        **kwargs,
        ...
        for i in range(num_layers):
            ...
            current_concat = concat if i < num_layers - 1 else False

            conv = GATv2Conv(
                in_dim,
                out_dim,
                heads=current_heads,
                concat=current_concat,
    ...
    def forward(
        ...
        for i in range(self.num_layers):
            if i == self.num_layers - 1 and return_attention_weights:
                x, (edge_index, attn_weights) = self.convs[i](
                    x,
                    edge_index,
                    edge_attr=edge_attr,
        ...
        return (x, (edge_index, attn_weights)) if return_attention_weights else x